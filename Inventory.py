...
class Inventory:
  ...
  def displayInventory(self):
    ...
    for product in self.inventory:
      print(product["Code"] + " " *
            (self.CODE_MAX_LENGTH - len(product["Code"])) + product["Name"] +
            " " * (self.NAME_MAX_LENGTH - len(product["Name"])) +
            product["Brand"] + " " *
            (self.BRAND_MAX_LENGTH - len(product["Brand"])) + product["Unit"] +
            " " * (self.UNIT_MAX_LENGTH - len(product["Unit"])) +
            product["Price"] + " " *
            (self.PRICE_MAX_LENGTH - len(product["Price"])))
  ...
  def displayPayment(self):
    ...
    print(
      f"TRANSACTION COMPLETE: RECEIPT NO. {random.randrange(10000000, 99999999)}\n"
    )
