...
class Inventory:
  """
  It initializes the class by setting the subtotal to 0, 
  and then sets the max length of each column in the inventory table.
  """
  def __init__(self):
    self.subTotal = 0
    ...
    with open('database.json') as f:
      ...
      self.inventory = self.database["inventory"]
  ...
  def displayInventory(self):
    print("CODE" + " " * (self.CODE_MAX_LENGTH - 4) + "NAME" + " " *
        (self.NAME_MAX_LENGTH - 4) + "BRAND" + " " *
        (self.BRAND_MAX_LENGTH - 5) + "UNIT" + " " *
        (self.UNIT_MAX_LENGTH - 4) + "PRICE" + " " *
        (self.PRICE_MAX_LENGTH - 5))
...
"""
  ...
  def addProduct(self):
    while (True):
      ...
      self.database["inventory"] = self.inventory;
    ...
    for i in range(0, len(self.inventory)):
      ...
      if (self.inventory[i]["Code"] == productCode):
        ...
        if (self.inventory[i]["Unit"] != "0"):
          self.inventory[i]["Unit"] = str(
              int(self.inventory[i]["Unit"]) - 1)
          self.subTotal += int(self.inventory[i]["Price"])
          print(self.inventory[i]["Name"] + " Added to Cart!")

          # Writing the inventory to the database.json file.
          self.database["inventory"] = self.inventory;
        ...
        else: print(self.inventory[i]["Name"] + " is out of stock!")
